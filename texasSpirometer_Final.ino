/**
 * File         texasSpirometer_final.ino
 * Date         July 16, 2022
 * The original code is modified from Rabbit Creek Githup repository https://github.com/rabbitcreek/TexasSpiromoter/blob/main/texasSpirometerCSV.ino Major additons were made to output flow volume loops, and calibrate the sensor.
 * This spirometer software is developed ONLY for educational use. It is not to be used for any human health related measurements.
 * 
 * Authors      Ian Murray  (imurray@tamu.edu)
 *              Brendan D'Souza(b11dsouza@tamu.edu)
 ...
 *            Disposabale Lily mputhpiece for Schiller SP-150/SP-250 Flow Sensor Spirometer https://www.schillerservice.com/product-p/2.100077.htm
 *            Tubing for pressure sensor ports
 *            I2C Qwiic cable female 4 pin with male pin male breadboard jumper wires
*            
 *            
 * Instructions: This program has 2 different ouput modes, an  you will need to change the int mode within the code to switch between each.  
 ...
 * 
       Note: The polarity of the schiller mouthpiece tube is important for the code (tests are triggered on expiration only- Mode 3). If a test is triggered on inhalation, then you know it’s backwards, then flip it.
 * 
Mode 1: This will output a basic spirogram via the Serial Plotter in the Arduino IDE (in the tab click on Tools --> Serial Monitor)

Mode 2: The data is output and viewed via the Serial Plotter in the Arduino IDE (in the tab click on Tools --> Serial Monitor)
    This mode is ready to collec data only when the data is scrolling across the page. If the data is not scrolling across the page, then the pressure sensor calibration step is being peformed. This caliraton step is needed, as the sensor is very sensitive 
...
Mode 3 will output text in the Serial Monitor, Starting calibration,and then indicating that you can start the test. You then put air into the device and the FEV1 and FVC are output, along with other calculated values.
See example below. It will continue giving values after the test, as the sensor is a bit buggy.
...
Mode 4 will output text in serial monitor, which can be outputted to MS Excel to plot a Flow-Volume loop.
*
...
void loop() {
    ...
    if(testing){
      ...
      if((TestTimer>1000)&&(FEV1needed)) {
        ...
        if(mode==3) // this will output the text data in mode 1
        {
          Serial.println("FEV1:");
          Serial.println(FEV1);
        }
